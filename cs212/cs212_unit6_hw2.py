...
LCASE = list(lowercase)
LETTERS = list('ABCDEFGHIJKLMNOPQRSTUVWXYZ') + LCASE
ANY = anchor(LETTERS + LCASE) # The anchor that can be any letter
                ...
                if L == '_':
                    for l in LCASE:
                        if l in possibilities:
                            add_suffixes(hand.replace('_', '', 1), pre+l, start, row, results)
                else:
                    if L in possibilities:
...
def find_prefixes(hand, pre='', results=None):
    ...
    if pre.upper() in PREFIXES:
        for L in hand:
            if L == '_':
                # Try all possible letters in this position.
                for l in LCASE:
                    find_prefixes(hand.replace('_', '', 1), pre+l, results)
            else:
