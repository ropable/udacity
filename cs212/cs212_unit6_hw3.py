...
def boggle_words(board, minlength=3):
    ...
    for i, l in enumerate(board):
        if l != '|':
            frontier.append((l, [i])) # Tuple: ('WORD', [list, of, used, indexes]
    while frontier:
        candidate = frontier.pop(0)
        if len(candidate[0]) >= minlength: # Consider this word.
            if candidate[0] in WORDS: all_words.add(candidate[0])
        for i in neighbors(candidate[1][-1], N): # Find all the neighbours to the last element.
            if board[i] != '|' and i not in candidate[1]: # Haven't used this letter yet.
                w = candidate[0] + board[i]
                if w in WORDS and len(w) >= minlength:
                    all_words.add(w)
                if w in PREFIXES:
                    frontier.append( (w, candidate[1] + [i]) )
    return all_words
